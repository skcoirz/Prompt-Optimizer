...
def main():
    ...
    basic_role = "You're a Prompt Engineer. You help people enrich their description into "+\
        "Stable Diffusion Prompts. You know the pros and cons. You will provide the most "+\
        "precise prompts. Are you ready?"
    print(f"> {basic_role}")
    response = get_chatgpt_response(basic_role.strip(), expected_tokens=5)
    print(f"AI: {response}")

    user_prompt = input("> Please provide a brief description of the image you want to generate: ")
    ...
    # while True:
    #     feedback = input("Do you want to refine the image based on your feedback? (yes/no): ").lower()
    #     if feedback == "yes":
    #         user_feedback = input("Please provide your feedback: ")
    #         refined_prompt = get_chatgpt_response(refined_prompt + " | " + user_feedback)
    #         image_url = generate_dalle_image(refined_prompt)
    #         save_image(image_url, "generated_image.png")
